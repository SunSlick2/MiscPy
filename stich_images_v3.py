...
COLUMN_SETS = [
    (30, 60),    # First column set (pixels 30 to 59 inclusive)
    (100, 150)   # Second column set (To be decided/changed later)
]
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    # Setting the fallback default if no argument is passed
    default_out = r"C:\Users\abc\Downloads\Test"

    parser = argparse.ArgumentParser(description="Stitch T/B image pairs from a child Raw directory.")
    parser.add_argument("outdir", nargs="?", default=default_out, 
                        help=f"Output directory for stitched files (Default: {default_out})")
    parser.add_argument("--threshold", type=float, default=MATCH_THRESHOLD)

    args = parser.parse_args()

    # Automatically derive the input directory structure: outdir\Raw
    input_dir = os.path.join(args.outdir, "Raw")

    pairs = find_pairs(input_dir)
    if not pairs:
        print(f"No matching T/B pairs found in {input_dir}")
        return
