...
def scan_for_leftover_macro_refs(contents: dict) -> list:
    ...
    patterns = [
        ...
        re.compile(r'xlm="1"', re.IGNORECASE),
        re.compile(r'<x:FmlaMacro>.*?</x:FmlaMacro>', re.IGNORECASE),
        re.compile(r'<x:MacroName>.*?</x:MacroName>', re.IGNORECASE),
    ]
    for name, data in contents.items():
        if not (name.endswith(".xml") or name.endswith(".rels") or name.lower().endswith(".vml")):
            continue
        try:
...
def strip_xlm_defined_names(contents: dict) -> dict:
    """Remove <definedName> entries with xlm="1" — these mark Excel 4.0
    (XLM) macro references, a separate legacy macro system from VBA that
    also triggers the 'contains macro-enabled content' warning."""
    wb_path = "xl/workbook.xml"
    if wb_path not in contents:
        return contents
    text = contents[wb_path].decode("utf-8")
    pattern = re.compile(r'<definedName[^>]*\bxlm="1"[^>]*>.*?</definedName>')
    new_text, n = pattern.subn("", text)
    if n > 0:
        print(f"    Removed {n} XLM (Excel 4.0 macro) defined name(s) from workbook.xml")
        contents[wb_path] = new_text.encode("utf-8")
    return contents


def strip_vml_macro_elements(contents: dict) -> dict:
    """Remove <x:FmlaMacro>...</x:FmlaMacro> / <x:MacroName>...</x:MacroName>
    elements from legacy VML drawing files (xl/drawings/vmlDrawingN.vml).
    Form Control buttons store their macro binding here as an ELEMENT, not
    an attribute — a completely different pattern from the macro="..."
    attribute used in the newer drawing/worksheet XML, so it needs its own
    stripping pass."""
    elem_pattern = re.compile(
        r'<x:(FmlaMacro|MacroName)>.*?</x:\1>', re.IGNORECASE | re.DOTALL
    )
    for name in list(contents.keys()):
        if not name.lower().endswith(".vml"):
            continue
        text = contents[name].decode("utf-8")
        new_text, n = elem_pattern.subn("", text)
        if n > 0:
            print(f"    Removed {n} VML macro element(s) from {name}")
            contents[name] = new_text.encode("utf-8")
    return contents


def strip_macro_attributes(contents: dict) -> dict:
    """Remove macro="..." attributes from any XML part (drawings, and
    worksheet XML which embeds inline fallback content for Form Control
    buttons — these carry their own macro="..." binding separate from
    the drawing parts)."""
...
def process_file(src_path: Path) -> Path:
    ...
    # Strip XLM (Excel 4.0) macro defined names from workbook.xml
    contents = strip_xlm_defined_names(contents)

    # Strip macro bindings from legacy VML drawing files (Form Control buttons)
    contents = strip_vml_macro_elements(contents)
