...
# Define the columns to compare as (start_pixel, end_pixel). 
# You can update the second tuple later when you decide on the range.
COLUMN_SETS = [
    (30, 45),    # First column set (pixels 30 to 59 inclusive)
    (45, 60)   # Second column set (To be decided/changed later)
]
# ─────────────────────────────────────────────────────────────────────────────
...
def find_overlap(top: np.ndarray, bot: np.ndarray,
    ...
    """
    Return the number of overlapping rows between the bottom of `top`
    and the top of `bot`.  Returns 0 when no overlap is detected.
    
    Compares only the column ranges specified in COLUMN_SETS.
    """
    h_top = top.shape[0]
    h_bot = bot.shape[0]
    ...
    for h in range(max_overlap, min_rows - 1, -1):
        ...
        # Extract and combine only the requested column slices
        tail_slices = np.hstack([tail[:, start:end] for start, end in COLUMN_SETS])
        head_slices = np.hstack([head[:, start:end] for start, end in COLUMN_SETS])
        
        diff = np.mean(np.abs(tail_slices - head_slices))
        if diff < threshold:
            return h
    return 0
...
def main():
    default_in = r"C:\Users\abc\Downloads\TestImages\Raw"
    default_out = r"C:\Users\abc\Downloads\TestImages"
    ...
    for top_path, bot_path, stem in pairs:
        out_path = os.path.join(args.outdir, f"{stem}0.png")
